// Convnet.JS file that can run an input on a trained network.
// This file is equivalent to the Javascript that runs on the example website
// at https://cs.stanford.edu/people/karpathy/convnetjs/demo/mnist.html
// with the only changes made after the search keyword MODIFICATIONS_HERE
// These changes are at the end of the file and essentially just allow the switch
// from an HTML Javascript setup to Duktape.
function input(params) {
  ...
  (function(lib) {
    ...
    // Below is the MNist network that was trained in a browser.
    // We assume that this trained network is given as input with proper weights for direct usage on test inputs.
    // var layer_defs = [];
    // layer_defs.push({type:'input', out_sx:1, out_sy:1, out_depth:2});
    // layer_defs.push({type:'fc', num_neurons:20, activation:'relu'});
    // layer_defs.push({type:'softmax', num_classes:10});
  
    // MNist from demo on website    
    // layer_defs = [];
    // layer_defs.push({type:'input', out_sx:24, out_sy:24, out_depth:1});
    // layer_defs.push({type:'conv', sx:5, filters:8, stride:1, pad:2, activation:'sigmoid'});
    // layer_defs.push({type:'pool', sx:2, stride:2});
    // layer_defs.push({type:'conv', sx:5, filters:16, stride:1, pad:2, activation:'sigmoid'});
    ...
    // MODIFICATIONS_HERE
    // Instead of loading from the example website, we load from our custom JSON
    var net = new convnetjs.Net(); // create an empty network
    net.fromJSON(params.network); // load all parameters from JSON

    var max = params.input.length;
    var start = 0;
    if (typeof params.start_index !== 'undefined') {
      // Make sure we do not go out of bounds
      start = Math.max(start, params.start_index);
    }
    if (typeof params.start_index !== 'undefined') {
      // Make sure we do not go out of bounds
      max = Math.min(max, params.end_index);
    }
    ...
    for (var index = start; index < max; index++) {
      curr_img = params.input[index];
      allimgs.push(curr_img);
      var x = new convnetjs.Vol(24,24,1,0.0);
      for(var i=0;i<W;i++) {
        //var ix = i; // * 4;
        x.w[i] = curr_img.image[i]/255.0;
      }
      processed_inputs.push(x.w);
      // x = convnetjs.augment(x, 24); // Do not augment for deterministic results

      // predict
      var prob = net.forward(x);
      allprobs.push(prob.w);

      // Pretty print the results
      var predicted_index = 0;
      for(var i = 0; i < prob.w.length; i++){
        if(prob.w[i] > prob.w[predicted_index]){
          predicted_index = i;
        }
      }
      var is_correct = false;
      if(predicted_index == curr_img.label){
        is_correct = true;
      }
      allresults.push({"predicted" : predicted_index, "probability": prob.w[predicted_index], "correct_label":curr_img.label, "correct":is_correct, "predictions": prob.w})
    }
  
    return '' + JSON.stringify({"results":allresults});//, "predictions": allprobs });//, "images": allimgs, "processed_images":processed_inputs});
  }
